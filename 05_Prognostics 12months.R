...
activity_levels <- c("AR","EER","DER","FAR")

# Original-FAR-Fälle bei T2 aus den Rohdaten holen
far_ids_T2 <- data %>%
  dplyr::filter(redcap_event_name == EV_T2_Q,
                activity_pattern == "FAR") %>%
  dplyr::pull(study_id) %>%
  unique()
...
dat_imp <- VIM::kNN(dat_pred, variable = vars_exist, k = 5) |>
  ...
  # Activity-Faktoren sauber setzen
  dplyr::mutate(
    activity_T1 = factor(activity_T1, levels = activity_levels),
    activity_T2 = factor(activity_T2, levels = activity_levels),
    activity_T3 = factor(activity_T3, levels = activity_levels)
  )

# FAR bei T2 hart aus den Rohdaten rekonstruieren (Pain)
dat_imp <- dat_imp %>%
  dplyr::mutate(
    activity_T2 = as.character(activity_T2),
    activity_T2 = ifelse(study_id %in% far_ids_T2, "FAR", activity_T2),
    activity_T2 = factor(activity_T2, levels = activity_levels)
  )
...
# FAR bei T2 hart aus den Rohdaten rekonstruieren (NDI)
dat_imp_ndi <- dat_imp_ndi %>%
  dplyr::mutate(
    activity_T2 = as.character(activity_T2),
    activity_T2 = ifelse(study_id %in% far_ids_T2, "FAR", activity_T2),
    activity_T2 = factor(activity_T2, levels = activity_levels)
...
## Optionaler Check, ob FAR noch da ist
cat("\n[Check] Verteilung Activity T2 (Pain):\n")
print(table(dat_imp$activity_T2, useNA = "ifany"))
cat("\n[Check] Verteilung Activity T2 (NDI):\n")
print(table(dat_imp_ndi$activity_T2, useNA = "ifany"))
...
table(act_T2$activity_T2)
summary(dat_imp_ndi$activity_T2)
...
# Pain-Range für DER-Teilnehmende bei T3----
data %>%
  filter(redcap_event_name == "fragebogen_t3_arm_1",
         activity_pattern == "DER") %>%
  summarise(
    n = n(),
    min = min(pain_avg, na.rm = TRUE),
    max = max(pain_avg, na.rm = TRUE),
    range = max - min,
    mean = mean(pain_avg, na.rm = TRUE),
    sd = sd(pain_avg, na.rm = TRUE)
  )
