...
colnames(data) <- c("study_id","redcap_event_name","redcap_repeat_instrument","redcap_repeat_instance","paindetect1","paindetect2",
                    ...
                    "pps7_strong","aefs_dms1","aefs_dms2","score_pps","dms_score","age", "gender", "cortisol2", "cortisone2",
                    "dropout", "smoking", "education", "occupation", "medication___0", "medication___1", "medication___2", 
                    "medication___3", "medication___4", "medication___5", "medication___6", "medication___na", "care___7",
                    "care___6", "care___5", "care___4", "care___3", "care___2", "care___1", "care___0", "care___na",
                    "sleepquality", "dass21_depression","dass21_anxiety", "dass21_stress", "met_vigorous","met_moderate",
                    "met_walk", "met_total", "total_sitting_mins","stais_score", "pvaq_score_total")

# Delete the first line
data <- data [-1, ]

# Define columns in which empty positions are to be replaced with NA
columns_to_fill <- c("study_id","redcap_event_name","redcap_repeat_instrument","redcap_repeat_instance","paindetect1","paindetect2",
                     "paindetect3","ndi1","ndi2","ndi3","ndi4","ndi5","ndi6",
                     "ndi7","ndi8","ndi9","ndi10","ndiscore","cortisol1","cortisone1","score_stress_skala_1",
                     "score_stress_skala_2","score_stress_skala_3","gesamtscore_symptom","sci_stress1_1","sci_stress1_2",
                     "sci_stress1_3","sci_stress1_4","sci_stress1_5","sci_stress1_6","sci_stress1_7","sci_stress2_1",
...
str(data_pain_T1$paindetect1)
str(data_pain_T1$paindetect2)
str(data_pain_T1$paindetect3)

str(data_pain_T2$paindetect1)
str(data_pain_T2$paindetect2)
str(data_pain_T2$paindetect3)

str(data_pain_T3$paindetect1)
str(data_pain_T3$paindetect2)
str(data_pain_T3$paindetect3)

str(data_pain_T4$paindetect1)
str(data_pain_T4$paindetect2)
str(data_pain_T4$paindetect3)
...
data_pain_T1 <- data_pain_T1 %>%
  mutate(across(c(paindetect1, paindetect2, paindetect3), as.numeric))

data_pain_T2 <- data_pain_T2 %>%
  mutate(across(c(paindetect1, paindetect2, paindetect3), as.numeric))

data_pain_T3 <- data_pain_T3 %>%
  mutate(across(c(paindetect1, paindetect2, paindetect3), as.numeric))

data_pain_T4 <- data_pain_T4 %>%
  mutate(across(c(paindetect1, paindetect2, paindetect3), as.numeric))
...
colSums(is.na(data_pain_T1[, c("paindetect1", "paindetect2", "paindetect3")]))
colSums(is.na(data_pain_T2[, c("paindetect1", "paindetect2", "paindetect3")]))
colSums(is.na(data_pain_T3[, c("paindetect1", "paindetect2", "paindetect3")]))
colSums(is.na(data_pain_T4[, c("paindetect1", "paindetect2", "paindetect3")]))
...
data_pain_T1 <- data_pain_T1 %>%
  mutate(pain_avg_T1 = rowMeans(select(., paindetect1, paindetect2, paindetect3), na.rm = TRUE))

data_pain_T2 <- data_pain_T2 %>%
  mutate(pain_avg_T2 = rowMeans(select(., paindetect1, paindetect2, paindetect3), na.rm = TRUE))

data_pain_T3 <- data_pain_T3 %>%
  mutate(pain_avg_T3 = rowMeans(select(., paindetect1, paindetect2, paindetect3), na.rm = TRUE))

data_pain_T4 <- data_pain_T4 %>%
  mutate(pain_avg_T4 = rowMeans(select(., paindetect1, paindetect2, paindetect3), na.rm = TRUE))


# Überprüfen, ob die neue Spalte korrekt erstellt wurde
head(data_pain_T1$pain_avg_T1)
head(data_pain_T2$pain_avg_T2)
head(data_pain_T3$pain_avg_T3)
head(data_pain_T4$pain_avg_T4)
...
data_stress_symptoms_T1 <- data %>%
  filter(redcap_event_name == "fragebogen_t1_arm_1") %>%
  dplyr::select(study_id, stress_skala_symptome_sum)
...
data_cortisol_T6 <- data %>%
  filter(redcap_event_name == "untersuchung_t6_arm_1") %>%
  dplyr::select(study_id, cortisol3 = cortisol1, cortisone3 = cortisone1, gender)
...
##Gender----
# Umwandeln der Spalte gender in numerische Werte (falls nötig)
data_gender$gender <- as.numeric(data_gender$gender)

# Nur gültige Werte berücksichtigen (1 = männlich, 2 = weiblich)
valid_genders <- data_gender$gender[data_gender$gender %in% c(1, 2)]
...
##smoking----
# Sicherstellen, dass smoking numerisch ist
data_smoking$smoking <- as.numeric(data_smoking$smoking)
...
##education----
# Sicherstellen, dass education numerisch ist
data_education$education <- as.numeric(data$education)
...
data_occupation$occupation <- as.numeric(data_occupation$occupation)
...
data_nomedication$medication_none <- as.numeric(dat_nomedicationa$medication_none)
data_painkillers$medication_painkillers <- as.numeric(data_painkillers$medication_painkillers)
data_opioids$medication_opioid <- as.numeric(data_opioids$medication_opioid)
data_antidepressants$medication_antidepressants <- as.numeric(data_antidepressants$medication_antidepressants)
data_musclerelaxants$medication_MmRelax <- as.numeric(data_musclerelaxants$medication_MmRelax)
data_cannabis$medication_Cann <- as.numeric(data_cannabis$medication_Cann)
...
data_medcare_none$care_none <- as.numeric(data_medcare_none$care_none)
data_medcare_generalpracticioner$care_GP <- as.numeric(data_medcare_generalpracticioner$care_GP)
data_medcare_specialist$care_Specialist <- as.numeric(data_medcare_specialist$care_Specialist)
data_physiotherapy$care_PT <- as.numeric(data_physiotherapy$care_PT)
data_chiropractic$care_Chiro <- as.numeric(data_chiropractic$care_Chiro)
data_psychiatrist$care_Psych <- as.numeric(data_psychiatrist$care_Psych)
data_massage$care_Mass <- as.numeric(data_massage$care_Mass)
...
# Schritt 1: Sicherstellen, dass paindetect-Spalten numerisch sind
data_pain_T1 <- data_pain_T1 %>% mutate(across(starts_with("paindetect"), as.numeric))
data_pain_T2 <- data_pain_T2 %>% mutate(across(starts_with("paindetect"), as.numeric))
data_pain_T3 <- data_pain_T3 %>% mutate(across(starts_with("paindetect"), as.numeric))
data_pain_T4 <- data_pain_T4 %>% mutate(across(starts_with("paindetect"), as.numeric))

# Schritt 2: Funktion zur Zusammenfassung
...
# Schritt 3: Anwendung auf alle Zeitpunkte
...
# Schritt 1: Sicherstellen, dass depression numerisch ist
data_depression_T1 <- data_depression_T1 %>% mutate(dass21_depression = as.numeric(dass21_depression))
data_depression_T2 <- data_depression_T2 %>% mutate(dass21_depression = as.numeric(dass21_depression))
data_depression_T3 <- data_depression_T3 %>% mutate(dass21_depression = as.numeric(dass21_depression))
data_depression_T4 <- data_depression_T4 %>% mutate(dass21_depression = as.numeric(dass21_depression))
...
##stress-scale: DASS21----
# Schritt 1: Umwandlung in numerisch
data_DASS_stress_T1 <- data_DASS_stress_T1 %>% mutate(dass21_stress = as.numeric(dass21_stress))
data_DASS_stress_T2 <- data_DASS_stress_T2 %>% mutate(dass21_stress = as.numeric(dass21_stress))
data_DASS_stress_T3 <- data_DASS_stress_T3 %>% mutate(dass21_stress = as.numeric(dass21_stress))
data_DASS_stress_T4 <- data_DASS_stress_T4 %>% mutate(dass21_stress = as.numeric(dass21_stress))

# Schritt 2: Zusammenfassungsfunktion
...
# Schritt 3: Zusammenfassung für alle Zeitpunkte
...
data_STAIanxiety_T1$stais_score <- as.numeric(data_STAIanxiety_T1$stais_score)
data_STAIanxiety_T2$stais_score <- as.numeric(data_STAIanxiety_T2$stais_score)
data_STAIanxiety_T3$stais_score <- as.numeric(data_STAIanxiety_T3$stais_score)
data_STAIanxiety_T4$stais_score <- as.numeric(data_STAIanxiety_T4$stais_score)
...
data_PVAQPainVigilance_T1$pvaq_score_total <- as.numeric(data_PVAQPainVigilance_T1$pvaq_score_total)
data_PVAQPainVigilance_T2$pvaq_score_total <- as.numeric(data_PVAQPainVigilance_T2$pvaq_score_total)
data_PVAQPainVigilance_T3$pvaq_score_total <- as.numeric(data_PVAQPainVigilance_T3$pvaq_score_total)
data_PVAQPainVigilance_T4$pvaq_score_total <- as.numeric(data_PVAQPainVigilance_T4$pvaq_score_total)
...
##Physical Activity----
# Sicherstellen, dass met_total numerisch ist
data_physicalactivity$met_total <- as.numeric(data_physicalactivity$met_total)
...
##Sedentary activity----
# Sicherstellen, dass total_sitting_mins numerisch ist
data_sedentaryactivity$total_sitting_mins <- as.numeric(data_sedentaryactivity$total_sitting_mins)
...
##Sleep----
# Sicherstellen, dass sleepquality numerisch ist
data_sleep$sleepquality <- as.numeric(data_sleep$sleepquality)
...
data_stress_uncertainty_T1$score_stress_skala_1 <- as.numeric(data_stress_uncertainty_T1$score_stress_skala_1)
data_stress_uncertainty_T2$score_stress_skala_1 <- as.numeric(data_stress_uncertainty_T2$score_stress_skala_1)
data_stress_uncertainty_T3$score_stress_skala_1 <- as.numeric(data_stress_uncertainty_T3$score_stress_skala_1)
data_stress_uncertainty_T4$score_stress_skala_1 <- as.numeric(data_stress_uncertainty_T4$score_stress_skala_1)
...
berechne_pps_dms_stats <- function(df, zeitpunkt = "T1") {
  ...
  df$score_pps <- as.numeric(df$score_pps)
  df$dms_score <- as.numeric(df$dms_score)
...
stats_cortisone_T1 <- berechne_stats_nach_gender(data_cortisol_T1, "cortisone1")

# T3
stats_cortisol_T3 <- berechne_stats_nach_gender(data_cortisol_T3, "cortisol2")
stats_cortisone_T3 <- berechne_stats_nach_gender(data_cortisol_T3, "cortisone2")

# T6
stats_cortisol_T6 <- berechne_stats_nach_gender(data_cortisol_T6, "cortisol3")
stats_cortisone_T6 <- berechne_stats_nach_gender(data_cortisol_T6, "cortisone3")
...
nur noch mit diesen Variablen rechnen:
data_pain_avg_T1 
data_pain_avg_T2 
data_pain_avg_T3 
data_pain_avg_T4
